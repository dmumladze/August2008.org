...
namespace August2008.Tests
{
    ...
    public class DonationServiceTests
    {
        ...
        [TestMethod]
        public void can_process_new_refunded_donation_ipn()   
        {
            var service = UnityHelper.Resolve<IDonationService>();
            var vars = this.CreatePayPalVariables(web_accept);
            vars.txn_id = ExternalId;
            ...
            vars = this.CreatePayPalVariables(refunded);
            vars.parent_txn_id = ExternalId;
            result = service.ProcessPayPalDonation(Encoding.ASCII.GetBytes(refunded), vars);
            Assert.IsTrue(result);
        }
        ...
        {
            var values = this.ParseFormVars(formVars);
            var type = typeof(PayPalVariables);
            var variables = new PayPalVariables();

            foreach (string item in values)
            {
                var prop = type.GetProperty(item, BindingFlags.Public | BindingFlags.Instance);
                if (prop != null)
                {
                    prop.SetValue(variables, HttpUtility.UrlDecode(values[item]));
                }
            }
            return variables;
        }
